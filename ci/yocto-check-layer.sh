...
git -c advice.detachedHead=false -c init.defaultBranch=master clone --quiet --shared $REPO_DIR meta-qcom-3rdparty

# Yocto Project layer checking tool
CMD="yocto-check-layer"
# Layer to check
CMD="$CMD meta-qcom-3rdparty"
...
CMD="$CMD --machines $(echo $(find meta-qcom-3rdparty/conf/machine/ -maxdepth 1 -name *.conf -exec basename {} .conf \; ))"
